Shader "Custom/CelluarNoise" {
	Properties {
		_Color ("Color", Color) = (1,1,1,1)
		_MainTex ("Albedo (RGB)", 2D) = "white" {}
		//Setting up variables for the custom noise functions
		_Amnt("Ammount", Range(-1,1)) = 0.5
		_Freq("Frequency", Range(0,50)) = 1.0
		_ClrAmnt("Color Ammount", Range(0,10)) = 0.5
		_ClrFreq("Color Frequency", Range(0,100)) = 1.0
	...
	SubShader {
		...
		uniform float _Amnt;
		uniform float _Freq;
		uniform float _ClrAmnt;
		uniform float _ClrFreq;
		float _Transparency;
		...
		{
			//Applying the noise function to the vertices
			float2 F = _Amnt * cellular(_Freq*float3(v.vertex.x, v.vertex.y, v.vertex.z));
			v.vertex.xyz += v.normal* 120 * (F.x - F.y);
		}

		//Fragment shader
		void surf (Input IN, inout SurfaceOutput o) {
			//Import the texture and multiply it with the color
			fixed4 c = tex2D (_MainTex, IN.uv_MainTex) * _Color;
			
			//Set albedo, which is the color of the shader
			o.Albedo = 2*c.rgb;
			//Apply the noise functions on all of the color channels individually
			//multiplying the final value with time to get an animation based on the tiling of the shader,
			//creating a water caustic effect
			o.Albedo.r += 0.3f*_ClrAmnt * cellular(_ClrFreq * float3(IN.uv_MainTex.x, IN.uv_MainTex.y, _Time.y*0.7f));
			o.Albedo.g += 0.3f*_ClrAmnt * cellular(_ClrFreq * float3(IN.uv_MainTex.x, IN.uv_MainTex.y, _Time.y*0.7f));
			o.Albedo.b += 0.3f*_ClrAmnt * cellular(_ClrFreq * float3(IN.uv_MainTex.x, IN.uv_MainTex.y, _Time.y*0.7f));
